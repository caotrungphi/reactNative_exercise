            ...
            [
                { text: "Bỏ qua", onPress: () => { console.log("Chọn No") }, style: "cancel" },
                {
                    text: "Đồng ý", onPress: () => {
                        ...
                        api.post(`${URL_SERVICE}/deleteUser`, {Ma_so})
                        .then((responseData) => {
...
export default class Cap_nhat_Danh_sach_Nguoi_dung extends Component {
    ...
    componentDidMount() {
        this.refs.Indicator.wait(1)
        api.get(`${URL_SERVICE}/listUser`).then(result => {
            dulieu.Danh_sach_Nguoi_dung = result;
            this.refs.Indicator.wait(2)
            this.setState({
                Danh_sach_Nguoi_dung: dulieu.Danh_sach_Nguoi_dung,
            })
            
        }).catch(err => {
            console.log(err)
            this.refs.Indicator.wait(3)
        })
    }
